...
covers(
    ...
    App\Http\Middleware\CacheUserActivity::class,
...
test('localization middleware applies the locale from the cookie and queues the default locale otherwise', function () {
...
test('cache user activity middleware stores the current user activity timestamp', function () {
...
test('handle inertia requests shares the current route and authenticated user name', function () {
...
test('redirect if authenticated sends signed in users to the home path', function () {
    $user = User::factory()->create();
    $this->actingAs($user);
...
test('redirect if authenticated passes guests through unchanged', function () {
    $request = Request::create('/', 'GET');
    $response = (new RedirectIfAuthenticated())->handle($request, fn () => response('ok'));
...
test('authenticate middleware redirects html guests to the start page', function () {
