...
covers(App\Library\Utility::class);
...
test('carbonize parses strings and preserves carbon instances', function () {
...
test('get carbon now applies configured timezone offset', function () {
...
test('create carbon date time combines date and time strings', function () {
    expect(Utility::createCarbonDateTime('03.06.2026', '14:30')->format('Y-m-d H:i:s'))
        ->toBe('2026-06-03 14:30:00');
});

test('send to log uses default log level and adds calling action', function () {
...
test('time string helpers parse date and time values', function () {
...
test('camel case conversion and login normalization are configurable', function () {
...
test('get translatable returns all locales and restores the original locale', function () {
    app()->setLocale('en');
