...
covers(App\Services\Console\CleanupIntervalResolver::class);
...
test('fromInstitution parses D:H:M format from institution setting', function () {
...
test('fromInstitution falls back to config when no cleanup_interval setting exists', function () {
    $institution = Institution::factory()->create();
...
test('fromInstitution handles days-only interval', function () {
    $institution = Institution::factory()->create();
    $institution->settings()->firstWhere('key', 'cleanup_interval')
...
test('fromValues subtracts each provided component', function () {
    $resolver = app(CleanupIntervalResolver::class);
...
test('fromValues handles null arguments by ignoring them', function () {
    $resolver = app(CleanupIntervalResolver::class);
...
test('fromValues handles empty string arguments as null', function () {
    $resolver = app(CleanupIntervalResolver::class);
...
test('fromValues handles integer arguments', function () {
    $resolver = app(CleanupIntervalResolver::class);
