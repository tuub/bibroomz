...
describe("Users/Form", () => {
    ...
    test("filters out institutions, roles, and assignments without ids", () => {
        const wrapper = render({
            user: {
                roles: [
                    { institution_id: "1", role_id: "10" },
                    { institution_id: "1", role_id: undefined },
                    { institution_id: undefined, role_id: "11" },
                ],
            },
            institutions: [{ id: "1", title: { en: "Alpha" } }, { title: { en: "Missing Id" } }],
            roles: [{ id: "10", name: { en: "Reader" } }, { name: { en: "Broken" } }],
        });
        const selects = wrapper.findAllComponents({ name: "MultiSelectStub" });
        const form = wrapper.findComponent({ name: "FormActionStub" }).props("form") as {
            roles: { institution_id: string; role_id: string[] }[];
        ...
        expect(selects[0]!.props("options")).toEqual([{ id: "10", name: { en: "Reader" } }]);
        expect(form.roles).toEqual([{ institution_id: "1", role_id: ["10"] }]);
    });
